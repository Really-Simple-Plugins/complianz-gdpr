...
if (!class_exists("CMPLZ_SERVICE")) {
    class CMPLZ_SERVICE
    {
        ...
        private function get($parent=false)
        {
            ...
            $sql = '';
            if ($parent) $sql = " AND isTranslationFrom = FALSE";

            if ($this->ID){
                $service = $wpdb->get_row($wpdb->prepare("select * from {$wpdb->prefix}cmplz_services where ID = %s ", $this->ID ));
            } else {
                $service = $wpdb->get_row($wpdb->prepare("select * from {$wpdb->prefix}cmplz_services where name = %s and language = %s ".$sql, $this->name, $this->language));
            }
        ...
        public function save($updateAllLanguages=false)
        {
            ...
            if ($updateAllLanguages){
                ...
                $serviceParent = new CMPLZ_SERVICE($isTranslationFrom);
                $translationIDS = $serviceParent->get_translations();
        ...
        public function get_translations(){
            ...
            $sql = $wpdb->prepare("select * from {$wpdb->prefix}cmplz_services where isTranslationFrom = %s", $parent_id);
