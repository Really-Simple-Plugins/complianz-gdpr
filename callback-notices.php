...
add_action('cmplz_notice_sensitive_information_processed', 'cmplz_notice_sensitive_information_processed');
function cmplz_notice_sensitive_information_processed(){
	if (cmplz_uses_sensitive_data()) {
		cmplz_notice(__("You have selected options that indicate your site processes sensitive, personal data. You should select 'Yes'", 'complianz-gdpr'));
	}
}
...
function cmplz_set_default($value, $fieldname)
{
    ...
    if ($fieldname == 'sensitive_information_processed') {
	    if (cmplz_uses_sensitive_data()) return 'yes';
    }
