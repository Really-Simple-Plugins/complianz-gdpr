...
add_filter( 'forminator_invalid_captcha_message', 'cmplz_forminator_override_recaptcha_error' ,10,4);
function cmplz_forminator_override_recaptcha_error($invalid_captcha_message, $element_id, $field, $verify){
    $invalid_captcha_message =__("Please accept cookies so we can validate your request with reCaptcha, and submit this form","complianz-gdpr");
...
function cmplz_forminator_add_consent_checkbox($form_id)
{
    ...
    $gdpr_field = Forminator_API::get_form_fields_by_type($form_id, 'gdprcheckbox');
    if (is_wp_error($gdpr_field)) {
        $data = array(
            ...
            'cols' => '12',
            'validation' => '',
            'gdpr_description' => sprintf(__("Yes, I agree with the %sprivacy statement%s","complianz-gdpr"),'<a href="' . COMPLIANZ()->document->get_permalink('privacy-statement') . '">', '</a>'),
