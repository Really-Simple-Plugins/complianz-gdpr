...
function cmplz_beehive_options($options, $network){
    ...
    if (cmplz_no_ip_addresses()) $options['general']['anonymize'] = TRUE;

    //handle sharing of data
    if (cmplz_statistics_no_sharing_allowed()) $options['general']['advertising'] = FALSE;
    return $options;
}
...
function cmplz_beehive_compile_statistics_more_info_notice()
{
    if (cmplz_no_ip_addresses()) {
        cmplz_notice(__("You have selected you anonymize IP addresses. This setting is now enabled in Beehive.", 'complianz-gdpr'));
    }
    if (cmplz_statistics_no_sharing_allowed()) {
        cmplz_notice(__("You have selected you do not share data with third party networks. Display advertising is now disabled in Beehive.", 'complianz-gdpr'));
    }
}
